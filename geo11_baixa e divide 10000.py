"""
================================================================================
  SENTINEL-2 L2A — DOWNLOAD + VERIFICACAO + CONVERSAO /10000
  Baixa, verifica o tipo de dado e ja divide uint16 por 10000
  Compatível com Windows | Python 3.8+
================================================================================
  DEPENDENCIAS:
    pip install pystac-client rasterio geopandas numpy requests tqdm pyproj shapely

  DIFERENCA do geo10:
    - Baixa os mesmos dados
    - Verifica se o tipo e uint16 ou float32
    - Se for uint16, ja divide por 10000 e salva como float32
    - Arquivos convertidos ganham sufixo "_refl.tif"
    - Os arquivos originais uint16 sao mantidos (ou apagados com flag)

  USO:
    python geo11.py

  CONFIGURACAO:
    Edite o bloco CONFIGURACOES DO USUARIO abaixo.
================================================================================
"""

# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║                         CONFIGURACOES DO USUARIO                             ║
# ║              Edite apenas este bloco antes de executar                       ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

from datetime import datetime as _dt

# ── Area e periodo ────────────────────────────────────────────────────────────
SHAPEFILE    = r"vetores/limite2.gpkg"
DATA_INICIO  = f"{_dt.now().year}-01-01"
DATA_FIM     = f"{_dt.now().year}-12-31"
MAX_NUVENS   = 50.0

# ── Selecao de imagens ────────────────────────────────────────────────────────
IMAGENS_SELECIONADAS = "interactive"

# ── Bandas para download ──────────────────────────────────────────────────────
BANDAS = None  # None = todas as bandas

# ── Recorte pela mascara do shapefile ─────────────────────────────────────────
RECORTAR = True

# ── Projecao de saida ─────────────────────────────────────────────────────────
EPSG_SAIDA = "utm"

# ── Pasta de saida ────────────────────────────────────────────────────────────
PASTA_SAIDA = r"ourives2"

# ── Conversao automatica ──────────────────────────────────────────────────────
# True  = se baixou uint16, ja divide por 10000 e salva como float32
# False = baixa normal sem converter (igual geo10)
CONVERTER_UINT16 = True

# True  = apaga os arquivos uint16 originais apos converter (economiza espaco)
# False = mantem os dois (original uint16 + convertido float32)
APAGAR_ORIGINAIS = False

# ══════════════════════════════════════════════════════════════════════════════

import os
import sys
import json
import time
import warnings
import traceback
from pathlib import Path

warnings.filterwarnings("ignore")

# ── VERIFICACAO E INSTALACAO DE DEPENDENCIAS ──────────────────────────────
def verificar_dependencias():
    pacotes = {
        "pystac_client": "pystac-client",
        "rasterio": "rasterio",
        "geopandas": "geopandas",
        "numpy": "numpy",
        "requests": "requests",
        "tqdm": "tqdm",
        "pyproj": "pyproj",
        "shapely": "shapely",
    }
    faltando = []
    for modulo, pip_nome in pacotes.items():
        try:
            __import__(modulo)
        except ImportError:
            faltando.append(pip_nome)

    if faltando:
        print("\n[AVISO] Pacotes ausentes detectados:")
        for p in faltando:
            print(f"  - {p}")
        resposta = input("\nDeseja instalar automaticamente agora? (s/n): ").strip().lower()
        if resposta == "s":
            import subprocess
            subprocess.check_call([sys.executable, "-m", "pip", "install"] + faltando)
            print("\n[OK] Pacotes instalados. Reiniciando importacoes...\n")
        else:
            print("\nInstale manualmente com:\n  pip install " + " ".join(faltando))
            sys.exit(1)

verificar_dependencias()

# ── IMPORTACOES PRINCIPAIS ────────────────────────────────────────────────
import numpy as np
import requests
import geopandas as gpd
import rasterio
from rasterio.mask import mask as rio_mask
from rasterio.warp import calculate_default_transform, reproject, Resampling
from pyproj import CRS, Transformer
from shapely.geometry import shape, mapping, box
from shapely.ops import transform as shapely_transform
import functools
from tqdm import tqdm
from pystac_client import Client

# ── CONSTANTES ────────────────────────────────────────────────────────────
STAC_URL     = "https://earth-search.aws.element84.com/v1"
COLLECTION   = "sentinel-2-l2a"
RESOLUCAO_M  = 10

ASSET_MAP = {
    "B01": ["coastal",   "B01"],
    "B02": ["blue",      "B02"],
    "B03": ["green",     "B03"],
    "B04": ["red",       "B04"],
    "B05": ["rededge1",  "B05"],
    "B06": ["rededge2",  "B06"],
    "B07": ["rededge3",  "B07"],
    "B08": ["nir",       "B08"],
    "B8A": ["nir08",     "B8A"],
    "B09": ["nir09",     "B09"],
    "B11": ["swir16",    "B11"],
    "B12": ["swir22",    "B12"],
    "SCL": ["scl",       "SCL"],
}

BANDAS_DISPONIVEIS = {
    "B01": ("Coastal Aerosol",         "60m"),
    "B02": ("Blue",                    "10m"),
    "B03": ("Green",                   "10m"),
    "B04": ("Red",                     "10m"),
    "B05": ("Red Edge",                "20m"),
    "B06": ("Red Edge",                "20m"),
    "B07": ("Red Edge",                "20m"),
    "B08": ("Near Infrared",           "10m"),
    "B8A": ("Red Edge",                "20m"),
    "B09": ("Water Vapour",            "60m"),
    "B11": ("SWIR 1",                  "20m"),
    "B12": ("SWIR 2",                  "20m"),
    "SCL": ("Scene Classification",    "20m"),
}

BANDAS_INFO = {}
for b, v in BANDAS_DISPONIVEIS.items():
    aliases = ASSET_MAP.get(b, [b])
    BANDAS_INFO[b] = {
        "aliases": aliases,
        "desc": v[0],
        "res": v[1],
        "enabled": True,
    }

BANDAS_DICT = {b: info["enabled"] for b, info in BANDAS_INFO.items()}

if BANDAS is None:
    BANDAS = BANDAS_DICT

COMPOSICOES_PRONTAS = {
    "1": {"nome": "RGB Natural",          "bandas": ["B04", "B03", "B02"]},
    "2": {"nome": "Falsa Cor (NIR)",      "bandas": ["B08", "B04", "B03"]},
    "3": {"nome": "SWIR (Vegetacao)",     "bandas": ["B12", "B8A", "B04"]},
    "4": {"nome": "Agricultura",          "bandas": ["B11", "B08", "B02"]},
    "5": {"nome": "Indice Urbano",        "bandas": ["B12", "B11", "B04"]},
    "6": {"nome": "Todas as bandas",      "bandas": list(BANDAS_INFO.keys())},
    "7": {"nome": "Escolher manualmente", "bandas": None},
}

SEP = "=" * 72


# ══════════════════════════════════════════════════════════════════════════════
#  FUNCOES AUXILIARES
# ══════════════════════════════════════════════════════════════════════════════

def cabecalho():
    print(f"\n{SEP}")
    print("   SENTINEL-2 L2A - DOWNLOAD + VERIFICACAO + CONVERSAO /10000")
    print("   STAC: Element 84 / AWS | Colecao: sentinel-2-l2a")
    print(SEP)


def secao(titulo: str):
    print(f"\n{'-' * 72}")
    print(f"  {titulo}")
    print(f"{'-' * 72}")


def ler_shapefile(caminho: str) -> tuple:
    caminho = Path(caminho)
    if not caminho.exists():
        raise FileNotFoundError(f"Arquivo nao encontrado: {caminho}")

    gdf = gpd.read_file(caminho)
    epsg_original = gdf.crs.to_epsg() if gdf.crs else None

    print(f"\n  [Shape] {caminho.name}")
    print(f"  [CRS original] EPSG:{epsg_original} - {gdf.crs.name if gdf.crs else 'desconhecido'}")
    print(f"  [Geometrias] {len(gdf)} feicoes | Tipo: {gdf.geom_type.iloc[0]}")

    if epsg_original != 4326:
        print(f"  [Info] Reprojetando para WGS84 (EPSG:4326) para busca STAC...")
        gdf_wgs84 = gdf.to_crs(epsg=4326)
    else:
        gdf_wgs84 = gdf

    return gdf, gdf_wgs84, epsg_original


def bbox_wgs84(gdf_wgs84: gpd.GeoDataFrame) -> list:
    bounds = gdf_wgs84.total_bounds
    return [round(float(v), 6) for v in bounds]


def buscar_imagens(bbox: list, data_inicio: str, data_fim: str, max_nuvens: float = 100.0) -> list:
    print(f"\n  [STAC] Conectando em {STAC_URL}...")
    catalog = Client.open(STAC_URL)
    print(f"  [Busca] Area: {bbox}")
    print(f"  [Busca] Periodo: {data_inicio} -> {data_fim}")
    print(f"  [Busca] Nuvens maximas: {max_nuvens}%")

    search = catalog.search(
        collections=[COLLECTION],
        bbox=bbox,
        datetime=f"{data_inicio}/{data_fim}",
        query={"eo:cloud_cover": {"lte": max_nuvens}},
        max_items=200,
    )

    itens = list(search.items())
    itens.sort(key=lambda i: i.properties.get("datetime", ""))
    return itens


def exibir_tabela_imagens(itens: list) -> None:
    secao("IMAGENS DISPONIVEIS")
    print(f"\n  {'#':>3}  {'DATA':^12}  {'TILE':^10}  {'NUVENS':>7}  {'PLATAFORMA':^12}  {'ID STAC'}")
    print(f"  {'-'*3}  {'-'*12}  {'-'*10}  {'-'*7}  {'-'*12}  {'-'*40}")

    for i, item in enumerate(itens, 1):
        props   = item.properties
        data    = props.get("datetime", "?")[:10]
        nuvens  = props.get("eo:cloud_cover", -1)
        tile_id = props.get("s2:mgrs_tile", props.get("mgrs:utm_zone", "?"))
        plat    = props.get("platform", "?").upper()
        stac_id = item.id[:40]

        if nuvens < 10:
            qual = "[OK]"
        elif nuvens < 30:
            qual = "[-]"
        elif nuvens < 60:
            qual = "[~]"
        else:
            qual = "[!]"

        print(f"  {i:>3}  {data:^12}  {tile_id:^10}  {nuvens:>6.1f}% {qual}  {plat:^12}  {stac_id}")

    print(f"\n  Total: {len(itens)} imagens encontradas\n")


def selecionar_itens_interativamente(itens: list) -> list:
    while True:
        escolha = input(
            "  Digite os numeros das imagens para baixar (ex: 1,3,5 ou 1-4), ou 'all' para todas: "
        ).strip().lower()
        if not escolha or escolha == "all":
            return itens
        try:
            selecionados = resolver_selecao(escolha, itens)
            if selecionados:
                return selecionados
            print("  [Aviso] Nenhuma imagem valida na selecao. Tente novamente.")
        except Exception as e:
            print(f"  [Erro] Selecao invalida: {e}")


def baixar_url(url: str, destino: Path, desc: str = "") -> bool:
    try:
        r = requests.get(url, stream=True, timeout=120)
        r.raise_for_status()
        total = int(r.headers.get("content-length", 0))
        with open(destino, "wb") as f, tqdm(
            desc=f"    {desc}",
            total=total,
            unit="B",
            unit_scale=True,
            unit_divisor=1024,
            ncols=70,
            leave=False,
        ) as bar:
            for chunk in r.iter_content(chunk_size=8192):
                f.write(chunk)
                bar.update(len(chunk))
        return True
    except Exception as e:
        print(f"  [Erro] Download falhou: {e}")
        return False


def reprojetar_raster(src_path: Path, dst_path: Path, epsg_destino: int):
    with rasterio.open(src_path) as src:
        if src.crs.to_epsg() == epsg_destino:
            return
        transform, width, height = calculate_default_transform(
            src.crs, CRS.from_epsg(epsg_destino), src.width, src.height, *src.bounds
        )
        kwargs = src.meta.copy()
        kwargs.update({
            "crs": CRS.from_epsg(epsg_destino),
            "transform": transform,
            "width": width,
            "height": height,
        })
        with rasterio.open(dst_path, "w", **kwargs) as dst:
            for i in range(1, src.count + 1):
                reproject(
                    source=rasterio.band(src, i),
                    destination=rasterio.band(dst, i),
                    src_transform=src.transform,
                    src_crs=src.crs,
                    dst_transform=transform,
                    dst_crs=CRS.from_epsg(epsg_destino),
                    resampling=Resampling.bilinear,
                )


def recortar_raster(src_path: Path, dst_path: Path, geometrias_wgs84, epsg_saida: int):
    with rasterio.open(src_path) as src:
        crs_raster = src.crs

        if crs_raster.to_epsg() != 4326:
            transformer = Transformer.from_crs("EPSG:4326", crs_raster.to_string(), always_xy=True)
            geoms_reproj = [
                mapping(shapely_transform(
                    functools.partial(transformer.transform),
                    shape(g)
                ))
                for g in geometrias_wgs84
            ]
        else:
            geoms_reproj = geometrias_wgs84

        out_image, out_transform = rio_mask(src, geoms_reproj, crop=True, nodata=0)
        out_meta = src.meta.copy()
        out_meta.update({
            "height": out_image.shape[1],
            "width":  out_image.shape[2],
            "transform": out_transform,
        })

    with rasterio.open(dst_path, "w", **out_meta) as dst:
        dst.write(out_image)

    if epsg_saida and crs_raster.to_epsg() != epsg_saida:
        tmp = dst_path.with_suffix(".tmp.tif")
        dst_path.rename(tmp)
        reprojetar_raster(tmp, dst_path, epsg_saida)
        tmp.unlink()


# ══════════════════════════════════════════════════════════════════════════════
#  NOVIDADE: CONVERSAO UINT16 -> FLOAT32  (/10000)
# ══════════════════════════════════════════════════════════════════════════════

def converter_uint16_para_float32(src_path: Path, dst_path: Path, fator_escala: float = 10000.0) -> bool:
    """
    Abre um GeoTIFF uint16, divide pelo fator de escala
    e salva como float32.
    Retorna True se converteu, False se pulou.
    """
    try:
        with rasterio.open(src_path) as src:
            # So converte se for uint16 (ou uint)
            if src.dtypes[0] not in ("uint16", "uint8", "uint32"):
                print(f"      [!] Tipo {src.dtypes[0]} nao precisa de conversao. Pulando.")
                return False

            # Ler dados
            dados = src.read()
            nodata = src.nodata

            # Converter para float32 e dividir
            dados_f32 = dados.astype(np.float32) / fator_escala

            # Se tem nodata, restaurar nodata como 0 ou -9999
            if nodata is not None:
                for b in range(dados.shape[0]):
                    mascara = dados[b] == nodata
                    dados_f32[b][mascara] = 0.0  # nodata vira 0 no float

            # Metadados atualizados
            meta = src.meta.copy()
            meta.update({
                "dtype": "float32",
                "nodata": 0.0,
            })

        # Salvar
        with rasterio.open(dst_path, "w", **meta) as dst:
            dst.write(dados_f32)

        return True

    except Exception as e:
        print(f"      [Erro] Falha na conversao: {e}")
        return False


def verificar_e_converter(caminho_original: Path, prefixo: str, banda: str,
                           converter: bool, apagar_orig: bool) -> Path:
    """
    Verifica o tipo do raster baixado.
    Se for uint16 e CONVERTER_UINT16=True, divide por 10000 e salva como _refl.tif.
    Retorna o caminho do arquivo final (original ou convertido).
    """
    if not converter:
        return caminho_original  # Mantem original

    with rasterio.open(caminho_original) as src:
        dtype = src.dtypes[0]

    # Nao converte SCL (classificacao) ou se ja for float
    if banda == "SCL" or dtype == "float32":
        return caminho_original

    if dtype in ("uint16", "uint8", "uint32"):
        # Nome do arquivo convertido
        nome_conv = f"{prefixo}_{banda}_refl.tif"
        caminho_conv = caminho_original.parent / nome_conv

        if caminho_conv.exists():
            print(f"      [OK] Convertido ja existe: {caminho_conv.name}")
            if apagar_orig and caminho_original.exists():
                caminho_original.unlink()
            return caminho_conv

        print(f"      [Conv] Convertendo {dtype} -> float32 (/10000)...")
        ok = converter_uint16_para_float32(caminho_original, caminho_conv)

        if ok:
            # Estatisticas do convertido
            with rasterio.open(caminho_conv) as ck:
                b = ck.read(1)
                print(f"      [OK] Reflectancia: min={b.min():.4f}, max={b.max():.4f}, media={b.mean():.4f}")

            if apagar_orig:
                caminho_original.unlink()
                print(f"      [Limp] Original apagado: {caminho_original.name}")
            return caminho_conv
        else:
            return caminho_original  # Falha, mantem original

    return caminho_original


def processar_item(item, bandas: list, gdf_wgs84: gpd.GeoDataFrame,
                   recortar: bool, epsg_saida, pasta: Path,
                   converter: bool, apagar_orig: bool):
    """Baixa, processa e converte um item STAC."""
    props   = item.properties
    assets  = item.assets

    partes  = item.id.split("_")
    plat    = partes[0] if len(partes) > 0 else props.get("platform", "S2x")
    tile_id = partes[1] if len(partes) > 1 else props.get("s2:mgrs_tile", "XX")
    data    = props.get("datetime", "")[:10].replace("-", "")
    prefixo = f"{data}_{tile_id}_{plat}"

    pasta_item = pasta / prefixo
    pasta_item.mkdir(parents=True, exist_ok=True)

    print(f"\n  [+] Processando: {prefixo}")
    print(f"      Nuvens: {props.get('eo:cloud_cover', '?'):.1f}%")

    geoms      = [mapping(g) for g in gdf_wgs84.geometry]
    resultados = []
    convertidos = 0

    for banda in bandas:
        chaves_possiveis = BANDAS_INFO.get(banda, {}).get("aliases", [banda])
        asset_key = None
        for chave in chaves_possiveis:
            if chave in assets:
                asset_key = chave
                break

        if asset_key is None:
            print(f"      [Aviso] Banda {banda} nao encontrada nos assets.")
            continue

        url           = assets[asset_key].href
        nome_arquivo  = f"{prefixo}_{banda}.tif"
        caminho_temp  = pasta_item / f"_tmp_{banda}.tif"
        caminho_final = pasta_item / nome_arquivo

        # Checar se ja existe versao convertida
        caminho_refl = pasta_item / f"{prefixo}_{banda}_refl.tif"
        if caminho_refl.exists() and converter:
            print(f"      [Pulando] {caminho_refl.name} ja existe (convertido).")
            resultados.append(caminho_refl)
            continue

        if caminho_final.exists() and not converter:
            print(f"      [Pulando] {nome_arquivo} ja existe.")
            resultados.append(caminho_final)
            continue

        if caminho_final.exists() and converter:
            # Ja existe original, converter
            print(f"      [Reuso] {nome_arquivo} existe, convertendo...")
            final = verificar_e_converter(caminho_final, prefixo, banda, converter, apagar_orig)
            resultados.append(final)
            if final != caminho_final:
                convertidos += 1
            continue

        desc_banda = BANDAS_INFO.get(banda, {}).get("desc", "?")
        print(f"      Download {banda} - {desc_banda}...")

        ok = baixar_url(url, caminho_temp, desc=banda)
        if not ok:
            continue

        try:
            if recortar:
                epsg_out = epsg_saida
                if epsg_out is None:
                    with rasterio.open(caminho_temp) as src:
                        epsg_out = src.crs.to_epsg()
                recortar_raster(caminho_temp, caminho_final, geoms, epsg_out)
                caminho_temp.unlink(missing_ok=True)
            else:
                epsg_out = epsg_saida
                if epsg_out is None:
                    with rasterio.open(caminho_temp) as src:
                        epsg_out = src.crs.to_epsg()
                if epsg_out:
                    reprojetar_raster(caminho_temp, caminho_final, epsg_out)
                    caminho_temp.unlink(missing_ok=True)
                else:
                    caminho_temp.rename(caminho_final)

            print(f"      [OK] Salvo: {caminho_final.name}")

            # Verificar tipo e converter se necessario
            final = verificar_e_converter(caminho_final, prefixo, banda, converter, apagar_orig)
            resultados.append(final)
            if final != caminho_final:
                convertidos += 1

        except Exception as e:
            print(f"      [Erro] ao processar {banda}: {e}")
            traceback.print_exc()
            caminho_temp.unlink(missing_ok=True)

    # Salvar metadados
    meta_path = pasta_item / f"{prefixo}_metadata.json"
    with open(meta_path, "w", encoding="utf-8") as f:
        json.dump({
            "id":              item.id,
            "data":            props.get("datetime"),
            "tile":            tile_id,
            "plataforma":      plat,
            "nuvens_%":        props.get("eo:cloud_cover"),
            "epsg_original":   props.get("proj:epsg"),
            "bandas_baixadas": [str(r.name) for r in resultados],
            "convertidos_/10000": convertidos,
            "apagar_originais": apagar_orig,
        }, f, indent=2, ensure_ascii=False)

    print(f"      [Meta] {meta_path.name}")
    return resultados


# ══════════════════════════════════════════════════════════════════════════════
#  FUNCOES DE RESOLUCAO
# ══════════════════════════════════════════════════════════════════════════════

def resolver_bandas(bandas_cfg) -> list:
    PRESETS = {
        "rgb":       ["B04", "B03", "B02"],
        "falsa_cor": ["B08", "B04", "B03"],
        "swir":      ["B12", "B8A", "B04"],
        "agri":      ["B11", "B08", "B02"],
        "urbano":    ["B12", "B11", "B04"],
        "todas":     list(BANDAS_INFO.keys()),
    }
    if isinstance(bandas_cfg, dict):
        selecionadas = []
        for b, v in bandas_cfg.items():
            if isinstance(v, dict):
                if v.get("enabled"):
                    selecionadas.append(b)
            else:
                try:
                    if bool(v):
                        selecionadas.append(b)
                except Exception:
                    continue
        return selecionadas

    if isinstance(bandas_cfg, (list, tuple)):
        out = []
        for item in bandas_cfg:
            if item is None:
                continue
            s = str(item).strip()
            if s.lower() in PRESETS:
                out.extend(PRESETS[s.lower()])
                continue
            if s in COMPOSICOES_PRONTAS:
                bandas_comp = COMPOSICOES_PRONTAS[s]["bandas"]
                if bandas_comp:
                    out.extend(bandas_comp)
                continue
            out.append(s.upper())
        vistos, unicos = set(), []
        for b in out:
            if b not in vistos:
                vistos.add(b)
                unicos.append(b)
        return unicos

    if isinstance(bandas_cfg, str):
        v = bandas_cfg.strip()
        if "," in v:
            parts = [p.strip() for p in v.split(",") if p.strip()]
            return resolver_bandas(parts)
        if v.lower() in PRESETS:
            return PRESETS[v.lower()]
        if v in COMPOSICOES_PRONTAS:
            comp = COMPOSICOES_PRONTAS[v]["bandas"]
            return comp if comp else []
        return [v.upper()]

    return []


def resolver_epsg(epsg_cfg, epsg_shapefile: int):
    if isinstance(epsg_cfg, int):
        return epsg_cfg
    v = str(epsg_cfg).strip().lower()
    if v == "shapefile":
        return epsg_shapefile
    if v == "utm":
        return None
    try:
        return int(v)
    except ValueError:
        raise ValueError(f"EPSG_SAIDA invalido: '{epsg_cfg}'")


def resolver_selecao(selecao_cfg: str, itens: list) -> list:
    v = str(selecao_cfg).strip().lower()
    if not v or v == "all":
        return itens
    if v in {"interactive", "choose", "prompt", "selecionar"}:
        return selecionar_itens_interativamente(itens)

    selecionados = []
    for parte in v.split(","):
        parte = parte.strip()
        if not parte:
            continue
        if "-" in parte:
            ini, fim = parte.split("-")
            ini = int(ini)
            fim = int(fim)
            if ini > fim:
                raise ValueError(f"Intervalo invalido: {parte}")
            for n in range(ini, fim + 1):
                if 1 <= n <= len(itens):
                    selecionados.append(itens[n - 1])
        else:
            n = int(parte)
            if 1 <= n <= len(itens):
                selecionados.append(itens[n - 1])

    if not selecionados:
        return []

    vistos, unicos = set(), []
    for item in selecionados:
        if item.id not in vistos:
            vistos.add(item.id)
            unicos.append(item)
    return unicos


# ══════════════════════════════════════════════════════════════════════════════
#  MAIN
# ══════════════════════════════════════════════════════════════════════════════

def main():
    cabecalho()

    # Mostrar config
    print(f"\n  Shapefile  : {SHAPEFILE}")
    print(f"  Periodo    : {DATA_INICIO} -> {DATA_FIM}")
    print(f"  Nuvens max : {MAX_NUVENS}%")
    print(f"  Recortar   : {'Sim' if RECORTAR else 'Nao'}")
    print(f"  EPSG saida : {EPSG_SAIDA}")
    print(f"  Saida      : {PASTA_SAIDA}")
    print(f"  Converter  : {'SIM (/10000 -> float32)' if CONVERTER_UINT16 else 'Nao (mantem uint16)'}")
    print(f"  Apagar orig: {'Sim' if APAGAR_ORIGINAIS else 'Nao'}")

    bandas = resolver_bandas(BANDAS)
    print(f"  Bandas     : {bandas}")

    # ── 1. Shapefile ─────────────────────────────────────────────────────────
    secao("1. LENDO SHAPEFILE")
    try:
        gdf_original, gdf_wgs84, epsg_original = ler_shapefile(SHAPEFILE)
    except FileNotFoundError as e:
        print(f"\n  [ERRO] {e}")
        sys.exit(1)

    epsg_saida = resolver_epsg(EPSG_SAIDA, epsg_original)
    epsg_label = epsg_saida if epsg_saida else "Nativo da cena (UTM)"
    print(f"  [Info] EPSG de saida resolvido: {epsg_label}")

    # ── 2. Buscar STAC ──────────────────────────────────────────────────────
    secao("2. BUSCANDO IMAGENS NO STAC")
    bbox = bbox_wgs84(gdf_wgs84)

    try:
        itens = buscar_imagens(bbox, DATA_INICIO, DATA_FIM, MAX_NUVENS)
    except Exception as e:
        print(f"\n  [ERRO] Falha na busca STAC: {e}")
        traceback.print_exc()
        sys.exit(1)

    if not itens:
        print(f"\n  [Aviso] Nenhuma imagem encontrada.")
        sys.exit(0)

    # ── 3. Tabela ───────────────────────────────────────────────────────────
    exibir_tabela_imagens(itens)

    # ── 4. Selecao ──────────────────────────────────────────────────────────
    try:
        itens_selecionados = resolver_selecao(IMAGENS_SELECIONADAS, itens)
    except Exception as e:
        print(f"\n  [ERRO] IMAGENS_SELECIONADAS invalido: {e}")
        sys.exit(1)

    if not itens_selecionados:
        print("\n  [Aviso] Nenhuma imagem valida na selecao.")
        sys.exit(0)

    # ── 5. Resumo ───────────────────────────────────────────────────────────
    secao("RESUMO DO DOWNLOAD")
    print(f"\n  Imagens selecionadas: {len(itens_selecionados)}")
    print(f"  Bandas              : {bandas}")
    print(f"  Converter /10000    : {'Sim' if CONVERTER_UINT16 else 'Nao'}")
    print(f"  Pasta de saida      : {PASTA_SAIDA}")

    pasta = Path(PASTA_SAIDA)
    pasta.mkdir(parents=True, exist_ok=True)
    print(f"\n  [OK] Arquivos serao salvos em: {pasta.resolve()}")

    # ── 6. Processar ────────────────────────────────────────────────────────
    secao("DOWNLOAD + CONVERSAO")
    total_arquivos = []
    total_convertidos = 0
    t0 = time.time()

    for idx, item in enumerate(itens_selecionados, 1):
        print(f"\n  [{idx}/{len(itens_selecionados)}]", end="")
        arquivos = processar_item(
            item, bandas, gdf_wgs84,
            RECORTAR, epsg_saida, pasta,
            CONVERTER_UINT16, APAGAR_ORIGINAIS
        )
        # Contar quantos sao _refl
        for arq in arquivos:
            total_arquivos.append(arq)
            if "_refl" in arq.name:
                total_convertidos += 1

    # ── 7. Relatorio ────────────────────────────────────────────────────────
    elapsed = time.time() - t0
    secao("CONCLUIDO")
    print(f"\n  [+] {len(total_arquivos)} arquivo(s) processados")
    if CONVERTER_UINT16:
        print(f"  [+] {total_convertidos} arquivo(s) convertidos para float32 (/10000)")
    print(f"  [Tempo] {elapsed/60:.1f} min")
    print(f"  [Pasta] {pasta.resolve()}")
    print(f"\n{SEP}\n")


if __name__ == "__main__":
    main()