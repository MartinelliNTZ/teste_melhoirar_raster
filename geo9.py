...
Pipeline (atualizado):
  ...
  2. Consulta o STAC e lista imagens disponíveis (data e % nuvem)
  3. Usuário escolhe uma cena
  4. Calcula o cubo grande que cobre toda a bbox (edge_size múltiplo de 128)
  5. Fatura em tiles 128×128 (com padding se necessário)
  6. Aplica o modelo SEN2SR em cada tile
  7. Remonta o mosaico
  8. Recorta exatamente no formato do polígono
"""
import os
import sys
import time
...
try:
    ...
    HAS_STAC_CLIENT = True
except ImportError:
    print("pystac-client não encontrado. Instalando...")
    import subprocess
    subprocess.check_call([sys.executable, "-m", "pip", "install", "pystac-client"])
    from pystac_client import Client
    HAS_STAC_CLIENT = True
...
def compute_with_retry(da, idx=0, max_tries=5, delay=2.0, backoff=2.0):
    """Força o carregamento do cubo com tentativas em caso de falhas de rede."""
...
def list_and_select_item(bbox, start_date, end_date, collection=COLECAO):
    """
    Consulta o STAC e retorna o item escolhido pelo usuário.
    ...
    while True:
        try:
            choice = input("\nEscolha o índice da imagem (ou Enter para a primeira): ").strip()
            if choice == "":
                idx = 0
                break
            idx = int(choice)
            if 0 <= idx < len(items):
                ...
                print(f"Índice fora do intervalo (0-{len(items)-1}).")
        except ValueError:
...
def main():
    ...
    # 2. Listar e escolher imagem
    selected_item = list_and_select_item(bbox, START_DATE, END_DATE)

    # 3. Calcular edge_size para cobrir toda a bbox (múltiplo de TILE_SIZE)
    ...
    # 5. Baixar cubo apenas para a imagem selecionada
    print(f"\nBaixando cubo S2 ({edge_px}×{edge_px}px, centro {cy:.6f}, {cx:.6f}) "
          f"para a cena {selected_item.id}...")
    # Usamos o item escolhido; datas ajustadas para garantir que ele seja capturado
    date_item = selected_item.datetime
    date_start = (date_item - np.timedelta64(1, 'D')).isoformat()
    date_end   = (date_item + np.timedelta64(1, 'D')).isoformat()
    ...
    # Download forçado
    cubo_np = compute_with_retry(da, idx=0)  # só há uma data no cubo
    ...
    # 9. Aplicar rio-color (melhoria de cores) no super-resolvido
    try:
